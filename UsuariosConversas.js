...
import { View, Text, TouchableOpacity, StyleSheet, FlatList, Image, ActivityIndicator, TextInput, Animated, Alert } from 'react-native';
...
const UsuariosConversas = () => {
    ...
    const deleteAnimations = useRef({}).current;
    ...
    const animateDelete = (userId) => {
        if (!deleteAnimations[userId]) {
            deleteAnimations[userId] = new Animated.Value(1);
        }
        const animation = deleteAnimations[userId];
        ...
        Alert.alert('Excluir usuário', 'Tem certeza que deseja excluir este usuário?', [
            ...
            },
            {
                ...
                onPress: async () => {
                    ...
                    const userDocRef = doc(db, 'users', currentUser.uid);
                    try {
                        // Remover o usuário da lista de usuários adicionados no banco
                        await updateDoc(userDocRef, {
                            addedUsers: arrayRemove(userId),
                        });
                        // Remover o usuário dos favoritos se ele estiver lá
                        await updateDoc(userDocRef, {
                            favorites: arrayRemove(userId),
                        });
                        
                        // Remover o usuário da lista local após a animação
    ...
    const renderUserItem = ({ item }) => {
        ...
        const deleteAnimation = deleteAnimations[item.id] || new Animated.Value(1);
        ...
        return (
            <TouchableOpacity onPress={() => openChat(item.id)} style={styles.userItem}>
                <View style={styles.tudoUsuarios}>
                    ...
                    >
                        <Animated.Image
                            source={require('./assets/icons/lixeiraImg.png')}
                            style={[
                                styles.deleteIcon,
